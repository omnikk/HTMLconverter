...
class CourseGenerator:
    ...
    def format_hours(self, hours):
        """Форматирует часы: целые числа без .0, дробные с запятой"""
        if hours == int(hours):
...
def process_all_csv_files(template_path, csv_folder_path, html_folder_path):
    ...
    if not Path(template_path).exists():
        return [], [(f"Шаблон не найден: {template_path}", "FileNotFoundError")]
    
    # Проверяем существование папки с CSV
    csv_folder = Path(csv_folder_path)
    if not csv_folder.exists():
        return [], [(f"Папка не найдена: {csv_folder_path}", "DirectoryNotFoundError")]
    
    os.makedirs(html_folder_path, exist_ok=True)
    csv_files = list(csv_folder.glob("*.csv"))
    
    if not csv_files:
        return [], [(f"CSV файлы не найдены в папке: {csv_folder_path}", "NoFilesFound")]
    ...
    for csv_file in csv_files:
        try:
            ...
            print(f"  ✓ Создан: {html_output_path.name}")
